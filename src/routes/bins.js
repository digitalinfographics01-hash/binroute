...
// GET /api/bins/:clientId/unmatched
router.get('/:clientId/unmatched', (req, res) => {
...
// GET /api/bins/:clientId/all
router.get('/:clientId/all', (req, res) => {
  const clientId = parseInt(req.params.clientId, 10);
  const rows = querySql(`
    SELECT
      b.bin, b.issuer_bank, b.card_brand, b.card_type, b.card_level, b.is_prepaid, b.source, b.last_updated,
      COUNT(o.id) as order_count,
      ROUND(COUNT(CASE WHEN o.order_status IN (2,6,8) THEN 1 END) * 100.0 /
        NULLIF(COUNT(CASE WHEN o.order_status IN (2,6,7,8) THEN 1 END),0),1) as approval_rate
    FROM bin_lookup b
    LEFT JOIN orders o ON o.cc_first_6 = b.bin AND o.client_id = ? AND o.is_test = 0 AND o.is_internal_test = 0
    GROUP BY b.bin
    ORDER BY order_count DESC
  `, [clientId]);
  res.json(rows);
...
// PUT /api/bins/:clientId/:bin
router.put('/:clientId/:bin', (req, res) => {
  const clientId = parseInt(req.params.clientId, 10);
  ...
  if (existing.length > 0) {
    ...
    // Resolve card_brand: use body value, or fall back to orders cc_type
    let resolvedBrand = card_brand || null;
    if (!resolvedBrand) {
      const orderRow = querySql(
        'SELECT cc_type FROM orders WHERE cc_first_6 = ? AND client_id = ? LIMIT 1',
        [bin, clientId]
      );
...
// GET /api/bins/:clientId/unmatched-count
router.get('/:clientId/unmatched-count', (req, res) => {
  const clientId = parseInt(req.params.clientId, 10);
  const row = querySql(`
