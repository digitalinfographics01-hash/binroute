/**
 * Backfill velocity features (Layer 2) for all tx_features rows.
 *
 * Usage:
 *   node scripts/backfill-velocity-features.js              — all clients
 *   node scripts/backfill-velocity-features.js --client=3   — client 3 only
 */
const { initDb, querySql, closeDb } = require('../src/db/connection');
const { initializeDatabase } = require('../src/db/schema');
const { computeVelocityFeatures } = require('../src/analytics/velocity-features');
...
(async () => {
  console.log(`=== Backfill Velocity Features (Layer 2)${CLIENT_ID ? ` — Client ${CLIENT_ID}` : ''} ===\n`);
  ...
  for (const client of clients) {
    ...
    console.log(`\n[${client.name}] Computing velocity features...`);
